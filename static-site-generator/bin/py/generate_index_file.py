...
def link_data(folder_path: str) -> List[Tuple[str, str]]:
    ...
    tmp.sort(
        key=lambda pair: os.path.getmtime(util.path(folder_path, pair[0])), reverse=True
    )
    return tmp
...
def create_index_file(temp_folder: str, notes_folder: str):
    ...
    with open(f"{temp_folder}/index.md", "w") as f:
        ...
        if len(todos) > 0:
            ...
            f.write("\n")
            f.write("\n")
            f.write("A collection of TODO items across all of your files.")
            f.write("\n")
            f.write("\n")
            f.write("<details>")
            ...
            f.write("\n")
            f.write(
                "When you're done with a task, delete the todo item from the file "
...
if __name__ == "__main__":
    ...
    create_index_file(temp_folder=args.temp, notes_folder=args.notes)

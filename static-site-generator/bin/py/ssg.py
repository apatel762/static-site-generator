...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="The relative path to the folder where the output HTML files " "will go",
    )
    ...
    broken_link_checker.check_for_broken_links(
        notes_folder=args.notes, cache_folder=args.temp
    )
    generate_backlinks_files.generate_backlinks_files(
        notes_folder=args.notes, backlinks_folder=args.temp
    )
    generate_index_file.create_index_file(
        temp_folder=args.temp, notes_folder=args.notes
    )
    pandocify.do_pandoc_generation(
        notes_folder=args.notes, temp_folder=args.temp, html_folder=args.html
    )
